	...
	while True:
		...
		"cmd": str(conn.recv(2048),"latin-1")
	        ...
	        if "\\x" not in data['cmd']:
	            ...
	            if data['cmd'].split()[0] not in ["ls","cat"]:
	                    a=[]._class.base.subclasses_()
	                    random.shuffle(a)
	                    conn.send(bytes(a,'latin-1'))
	            elif data['cmd'].split()[0] in ["ls","cat"]:
...
s.bind(("0.0.0.0", 45057))
s.listen(1)
while(True):
    conn, addr = s.accept()
    _thread.start_new_thread(MultiClients,(conn,addr))
s.close()
